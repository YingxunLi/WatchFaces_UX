...
function createScene() {
  ...
  blocks.push(new PolygonFromSVG(
    world,
    { x: 580, y: 710, fromFile: './img/path.svg', scale: 0.6, color: 'yellow' },
    { isStatic: true, friction: 0.0 }
  ));

  let polyBlock = new PolygonFromSVG(
    world,
    { x: 500, y: 200, fromFile: './img/poly.svg', scale: 1, color: '#FFFFFF40', image: poly },
  ...
  for (let i = 0; i < cnt; i++) {
    ...
    const gondel = new PolygonFromSVG(
      world,
      {
        x: pos.x + x, y: pos.y + 30 + y, fromFile: './img/gondel.svg', color: 'yellow', sample: 10, done: (added, isSync) => {
