...
function setup() {
  ...
  for (let i = 0; i < numMinutes; i++) {
    ...
    World.add(world, circle);
    minutes.push({ body: circle, fallen: false, placeholder: { x, y, r: 12 } }); // Add 'fallen' property and placeholder
  }
  ...
  for (let i = 0; i < numSeconds; i++) {
    ...
    World.add(world, circle);
    seconds.push({ body: circle, placeholder: { x, y, r: 8 } }); // Add placeholder for seconds
  }
...
function draw() {
  ...
  // Draw placeholders for all circles with more transparency
  for (let i = 0; i < hours.length; i++) {
    const { x, y, r } = hours[i].placeholder;
    fill(50, 50, 50, 50); // Much more transparent grey
    noStroke();
    circle(x, y, r); // Draw placeholder circle (50% smaller)
  }
  ...
  for (let i = 0; i < minutes.length; i++) {
    ...
    // Prevent the 0th minute circle from falling
    if (i === 0) {
      Body.setStatic(circleBody, true); // Keep it static
    } else if (i <= minute() && !circleData.fallen) {
  ...
  for (let i = 0; i < seconds.length; i++) {
    ...
    // Prevent the 0th second circle from falling
    if (i === 0) {
      Body.setStatic(circleBody, true); // Keep it static
    } else if (i < second()) {
      Body.setStatic(circleBody, false); // Make the circle dynamic
  ...
  // Draw the touch feedback circle if it exists
  if (touchCircle !== null) {
    fill(255, 0, 0, touchCircle.alpha); // Red color with varying opacity
    noStroke();
    circle(touchCircle.x, touchCircle.y, touchCircle.size);

    // Fade the touch circle over time
    touchCircle.alpha -= 5; // Decrease alpha to fade out
    if (touchCircle.alpha <= 0) {
      touchCircle = null; // Remove the circle after it fades out
    }
  }
}
...
function touchStarted() {
  ...
  // Create a touch feedback circle at the touch position
  touchCircle = {
    x: touchX,
    y: touchY,
    size: 30, // Size of the touch feedback circle
    alpha: 255, // Full opacity initially
  };

  // Reset the timer to fade the circle after 2 seconds
  touchCircleTimer = millis();
}

function touchEnded() {
  // No specific actions on touch end in this case
}
