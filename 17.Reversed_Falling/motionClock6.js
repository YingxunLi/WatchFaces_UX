...
function draw() {
  ...
  for (let i = 0; i < minutes.length; i++) {
    ...
    if (i <= currentMinute && !circleData.fallen) {
  ...
  // Handle falling of minutes (fall a minute when all second markers are done)
  if (currentSecond === 0 && !firstFallDone) {
    firstFallDone = true; // Mark first fall as done
  }

  // After all second circles have fallen, a minute circle should fall
  if (currentSecond === 0 && seconds.every(s => s.body.isStatic === false)) {
    // Make the next minute circle fall
    for (let i = 0; i < minutes.length; i++) {
      const circleData = minutes[i];
      const circleBody = circleData.body;

      if (i === currentMinute) {
        Body.setStatic(circleBody, false); // Make the current minute circle fall
        break;
      }
    }
  }
