...
const radiusMinutes = 250; // Der Radius des Minutenkreises (250px)
...
function setup() {
  ...
  for (let i = 0; i < numHours; i++) {
    ...
    const circle = Bodies.circle(x, y, 30, {
      ...
      frictionAir: 0.01, // Der Luftwiderstand, wie schnell der Kreis stoppt
  ...
  for (let i = 0; i < numMinutes; i++) {
    const angle = (TWO_PI / numMinutes) * i - 7 * PI / 15; // Berechnet den Winkel für jede Minutenmarkierung
    ...
    const circle = Bodies.circle(x, y, 12, {
      ...
      frictionAir: 0.01, // Luftwiderstand für Minutenmarkierungen
  ...
  for (let i = 0; i < numSeconds; i++) {
    ...
    const circle = Bodies.circle(x, y, 8, {
      ...
      frictionAir: 0.05, // Luftwiderstand für Sekundenmarkierungen
...
function draw() {
  ...
  fill(255); // Eine graue Farbe für den Mittelpunkt
  noStroke();
  circle(center.position.x, center.position.y, 80); // Kreis mit doppeltem Radius, weil p5.js `diameter` nutzt
  ...
  for (let i = 0; i < hours.length; i++) {
    ...
    fill(hours[i].fallen ? '#FF0000' : 150); // Rot für aktive Stunden
    noStroke();
